      ...
      <div className="bg-white p-4 lg:p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-lg transition-all duration-300">
          ...
          <div className="relative z-10">
              ...
              <div className="flex items-center gap-1.5 lg:gap-2">
                  ...
                  <span className="text-[9px] lg:text-[10px] text-slate-400 font-medium truncate">vs last week</span>
              </div>
          </div>
      </div>
  );
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
            <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-slate-800 text-sm">{isOwner ? 'Team Sales Target' : 'My Sales Target'}</h3>
                        <p className="text-slate-400 text-[10px]">Current Month Progress</p>
                    </div>
                </div>
    ...
    <div className="space-y-6 lg:space-y-8 relative pb-24 lg:pb-12">
      ...
      {backupStatus === 'success' && (
          <div className="fixed top-20 right-6 bg-white border border-emerald-100 p-4 rounded-xl shadow-xl flex items-center gap-3 z-50 animate-in slide-in-from-right duration-500">
              ...
              <div>
                  <h4 className="text-sm font-bold text-slate-800">System Backup</h4>
                  <p className="text-xs text-slate-500">Data successfully synced.</p>
              </div>
          ...
          <div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-6 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in duration-300">
                  <div className="bg-red-600 p-8 text-white text-center">
                      ...
                      <h2 className="text-2xl font-bold">Offline Access Restricted</h2>
                      <p className="text-red-100 text-sm mt-2">Connection Required</p>
                  </div>
                  <div className="p-8 text-center space-y-6">
                      <div className="p-4 bg-orange-50 text-orange-800 rounded-xl text-sm border border-orange-100 text-left">
                          <p className="font-bold flex items-center gap-2 mb-2">
                              <AlertTriangle className="w-4 h-4" /> Subscription Notice
                          </p>
                          <p className="leading-relaxed">
                              You have been using the free version for more than 14 days. Offline access is now limited to premium users.
                          </p>
                      </div>
                      <button 
          ...
          <div className="flex flex-col gap-3">
              ...
              <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-2">
                  <div className="flex gap-1 p-1 bg-slate-50 rounded-xl shrink-0">
                      <button onClick={() => setFilterMode('day')} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${filterMode === 'day' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Day</button>
                      <button onClick={() => setFilterMode('month')} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${filterMode === 'month' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Month</button>
                      <button onClick={() => setFilterMode('year')} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${filterMode === 'year' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Year</button>
                  </div>
              ...
              <div className="relative">
                ...
                >
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                        <span className="text-sm font-bold text-slate-800 truncate">{user?.businessName || 'My Business'}</span>
                        <div className="flex items-center gap-1.5">
                            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span className="text-xs text-slate-500 font-medium whitespace-nowrap">Synced {lastBackupTime}</span>
                        </div>
                    </div>
                ...
                {showBusinessSwitcher && user?.businesses && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                        <p className="text-[10px] font-bold text-slate-400 uppercase px-3 py-2">Switch Business</p>
                ...
                <div className="absolute inset-0 flex items-center justify-between p-8 text-white z-10">
                    <div className="max-w-[70%]">
                        ...
                        <button className="bg-white text-slate-900 px-6 py-2.5 rounded-xl font-bold shadow-lg hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all text-sm flex items-center gap-2">
                            Explore Now <ArrowRightLeft className="w-4 h-4" />
                        </button>
                    </div>
                </div>
              ...
              <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-2xl p-6 text-white relative overflow-hidden shadow-xl group cursor-pointer hover:shadow-2xl transition-all duration-300" onClick={() => navigate('/ai-chat')}>
                  ...
                  <div className="relative z-10">
                      <div className="flex items-center gap-2 mb-4">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider text-indigo-200">AI Hub</span>
                      </div>
                      <h3 className="text-xl font-bold mb-2">Business Assistant</h3>
                      <p className="text-indigo-200 text-sm mb-6 leading-relaxed">Ask Gemini to analyze your sales, generate marketing copy, or predict stock needs.</p>
                      <button className="w-full py-3 bg-white text-slate-900 rounded-xl font-bold text-sm hover:bg-indigo-50 transition-colors flex items-center justify-center gap-2">
                          <Bot className="w-4 h-4" /> Open Chat
                      </button>
                  </div>
              </div>
                  ...
                  <div className="relative z-10">
                      <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                              ...
                              <div>
                                  ...
                                  <p className="text-slate-400 text-[10px]">Valid until Dec 31, 2024</p>
                              </div>
                          </div>
                      ...
                      <div className="space-y-4">
                          <div className="space-y-2">
                              ...
                              <p className="text-xs text-slate-400 mt-2 text-center">1500 / 2000 AI Requests used</p>
                          </div>
                          <button 
